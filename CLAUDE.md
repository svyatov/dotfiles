...
- Every skill's `description` frontmatter is injected into the system prompt on every session, so an unused enabled plugin costs tokens continuously, not just disk. Check usage before adding one: `grep -oh '"skill":"[^"]*"' ~/.claude/projects/*/*.jsonl | sort | uniq -c | sort -rn`. The listing is capped by `skillListingBudgetFraction` (default `0.01` of the context window) with a per-skill cap of `skillListingMaxDescChars` (default `1536`); past the budget, the least-used skills lose their descriptions and list by name only. `/doctor` estimates the listing cost against the budget.
