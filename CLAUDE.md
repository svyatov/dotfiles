...
- `claude/commands/` is symlinked as a **whole directory** onto `~/.claude/commands`, unlike `claude/skills/`, because nothing else writes there: plugin commands live under `~/.claude/plugins/`. Every `.md` file in it becomes a slash command named after the file, so documentation belongs in `README.md`, not in that directory. `setup.sh` has to `rmdir` the live path first: Claude Code creates it as a real empty directory, and `ln -h` refuses to replace one. `claude/output-styles/` gets the identical whole-directory treatment, `rmdir` included; the `outputStyle` key in `claude/settings.json` selects the active style.
...
- `~/.claude/settings.json` is the one config that is **synced, not symlinked**. Claude Code rewrites it at runtime and supacode injects hooks into it; supacode writes atomically, which replaces a symlink with a regular file. `claude/settings-sync.rb` reconciles instead. Both copies drift in both directions, so it diffs them at leaf-path granularity (string arrays such as `permissions.deny` compare element-wise) and asks which side wins for each difference, then writes both files. Only two things resolve without asking, and both are reported: hooks matching `supacode-managed-hook|orca|handrail` stay live and out of the repo, and pruned `enabledPlugins` entries keep their repo value. `setup.sh` no longer pushes; it installs the repo copy only when no live file exists. The old `--pull`/`--push` modes each dropped every top-level key that existed on the other side only, which is how `autoCompactWindow` was lost.
