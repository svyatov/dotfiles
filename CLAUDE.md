...
- Claude Code drops `enabledPlugins` entries for plugins it cannot resolve, so a missing key there is never a decision (disabling writes `false`). `settings-sync.rb` keeps the repo entry without prompting, and says that it did.
