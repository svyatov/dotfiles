...
safe_alias eam 'v ~/.dotfiles/zsh/aliases_mise.sh'
...
safe_alias aiy 'claude --dangerously-skip-permissions'
...
safe_alias syu 'ewu mise rtk && mup && npm install -g npm@latest && bun upgrade && uv self update && npm install -g codegraph@latest'
...
# YouTube-dl
safe_alias ytd "yt-dlp --ignore-errors --continue --format \"bestvideo[ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]/best[ext=mp4]\" --cookies c.txt"
